    ...
    public class InfiniteTerrainGenerator : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (_mapGenerator.generateInfiniteTerrain)
            {
                ...
                // Initialize NavMesh with terrain parent
                AsyncNavMeshBuildScheduler.Initialize(parent);
            ...
            for (int yOffset = -_chunksVisibleInViewDistance; yOffset <= _chunksVisibleInViewDistance; yOffset++)
            {
                for (int xOffset = -_chunksVisibleInViewDistance; xOffset <= _chunksVisibleInViewDistance; xOffset++)
                {
                    ...
                    else
                    {
                        ...
                        TerrainChunk newChunk = new TerrainChunk(viewedChunkCoord, _chunkSize, LevelOfDetailLevels,
                            parent, meshMaterial);
